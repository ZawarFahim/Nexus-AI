...
CTRL_SHIFT_SPACE: Final = Chord(
    win32.VK_SPACE, frozenset({Modifier.CTRL, Modifier.SHIFT})
)
...
class HotkeyListener:
    ...
    def _hook_procedure(self, code: int, wparam: int, lparam: int) -> int:
        ...
        # Nexus synthesises keystrokes to drive the browser, and its own hook sees
        ...
        # push-to-talk -- Nexus interrupting itself with a key it pressed. Passing
        # injected events straight through also means Nexus cannot be driven by
    ...
    def _classify(self, vk_code: int, message: int) -> bool:
        ...
        Returns:
            True if Nexus owns this keystroke and it should be hidden from other
            applications.
        """
