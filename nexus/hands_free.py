...
for every sentence. Toggling this on lets Nexus listen continuously, work out
when each sentence ends, answer, and keep listening.

The complication is that Nexus can hear itself. On speakers its own reply would
be detected as speech and answered, so the microphone is ignored while Nexus is
...
from nexus.audio.endpointing import SpeechEndpointer, SpeechEvent
...
class HandsFreeMode:
    ...
    Args:
        ...
        state: Shared state, used to know when Nexus is talking.
    ...
    def enabled(self) -> bool:
        """Whether Nexus is listening continuously."""
    ...
    def _process(self, block: np.ndarray) -> None:
        ...
        if self._is_output_active():
            # Nexus is talking or thinking. Feeding this in would either detect
            # Nexus's own voice on speakers, or start a second utterance while
