...
from nexus.audio.devices import DeviceInfo
from nexus.audio.player import StreamingPlayer
...
# Nexus had finished, cutting it off before its first word.
...
class PiperVoice:
    ...
    Satisfies :class:`~nexus.core.protocols.TTSEngine`.
    ...
    def speak_stream(self, fragments: Iterable[str]) -> None:
        ...
        Sentences are synthesised as they complete, so Nexus begins speaking while
    ...
    def _await_idle(self, timeout: float = SPEAK_TIMEOUT_SECONDS) -> bool:
        ...
        cutting Nexus off before its first word.
