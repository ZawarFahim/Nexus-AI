...
The sentence is the unit that makes Nexus feel responsive. A language model emits
fragments a few characters at a time; a synthesiser needs a whole sentence to
produce natural intonation. Splitting on sentence boundaries is what lets Nexus
...
before Nexus makes a sound, and that silence is the thing users actually notice.
"""
    ...
    *,
    ...
    Args:
        fragments: Arbitrary chunks of text, in order, such as those produced
            by :meth:`~nexus.core.protocols.LLMProvider.stream_reply`.
