...
The single place in Nexus that knows which services exist. Everything else
depends only on :class:`~nexus.core.protocols.LLMProvider`, so a chain of three
...
signups before Nexus says a word would undo the setup flow; but a user who runs
...
from nexus.core import credentials
from nexus.core.config import Settings
from nexus.core.protocols import LLMError, LLMProvider, Message
from nexus.llm import providers
from nexus.llm.chain import ProviderChain
from nexus.llm.openai_compatible import OpenAICompatibleProvider
from nexus.llm.providers import ProviderSpec
...
def configured(settings: Settings) -> list[ProviderSpec]:
    ...
    The one named in settings leads, whether or not it is Nexus's own default.
    """
...
def create_provider(settings: Settings, api_key: str = "") -> LLMProvider:
    """Build the chain of providers Nexus will use.
    ...
    Args:
        ...
        api_key: Key for the primary provider, overriding what is stored.
            ...
            encrypted store before Nexus starts.
    ...
    if not specs:
        raise LLMError(
            f"No API key for {primary.label}. Run Nexus once interactively, or set "
            f"{primary.env_var}."
        )
...
def validate_key(provider_name: str, api_key: str) -> str:
    ...
    than three minutes later when Nexus mysteriously will not answer.
