"""The services Nexus can think with, and what each one is good for.
...
out, and Nexus then goes quiet until tomorrow. Three tiers used in order is not a
...
which is what makes Nexus feel like a conversation rather than a form submission.
...
class ProviderSpec:
    ...
    Attributes:
        ...
        reasoning_effort: Sent with vision requests when set. See
            :mod:`nexus.llm.thinking` for why this matters.
...
CEREBRAS: Final = ProviderSpec(
    ...
    # wrong kind. See _explain_payment_required in nexus.llm.factory.
    signup_note=(
...
GEMINI: Final = ProviderSpec(
    ...
    # The failure lands *after* the tool has run, so a browser opened and Nexus
...
# which is what makes Nexus feel like a conversation rather than a form.
