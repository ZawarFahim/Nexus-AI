...
interface, so one implementation covers every provider Nexus is likely to want,
...
generic and :mod:`nexus.llm.groq` is four lines.
"""
...
# does not leave the user staring at nothing. Nexus has no way to show progress.
...
def _message_payload(message: Message) -> dict[str, Any]:
    ...
    Four shapes exist, and only the first is what most of Nexus ever produces:
...
# into something Nexus would actually say.
BLIND_NOTE: Final = (
...
def _tool_exchange_as_text(
    ...
    Nexus that never says anything.
...
def _classify(exc: OpenAIError, label: str) -> LLMError:
    """Translate an SDK exception into an error Nexus can speak about."""
...
class OpenAICompatibleProvider:
    ...
    Satisfies :class:`~nexus.core.protocols.LLMProvider`.
    ...
    Args:
        ...
        temperature: Sampling temperature. Slightly above zero keeps Nexus from
    ...
    def stream_reply(
        ...
        :mod:`nexus.llm.thinking` for why that belongs here and not in the caller.
    ...
    def _stream_once(
        ...
        if seeing and not self.can_see:
            ...
            # explanation lets the turn finish: Nexus says it cannot see right
    ...
    def warm_up(self) -> bool:
        ...
        the one that decides whether Nexus feels responsive.
        ...
        # model without saying it is gone -- which reads as Nexus being broken
        ...
        if available and self._model.rsplit("/", 1)[-1] not in available:
            logger.warning(
                "%s does not list model %r. Set NEXUS_LLM_MODEL to one of: %s",
                self._label,
                self._model,
