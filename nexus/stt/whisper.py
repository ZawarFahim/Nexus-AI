...
Hardware is detected at runtime rather than configured. Nexus is meant to be
...
from nexus.core import assets, paths
from nexus.core.protocols import Transcript
from nexus.stt import runtime
...
# Whisper has never seen Nexus's name and reliably hears "Eevee". A short prompt
...
def detect_profile(preference: str = "auto") -> ModelProfile:
    ...
    if preference == "cuda":
        return GPU_PROFILE
    if preference != "auto":
        logger.warning("Unknown NEXUS_DEVICE=%r; using auto detection", preference)
...
class WhisperTranscriber:
    """Transcribes microphone audio. Satisfies :class:`~nexus.core.protocols.STTEngine`.
    ...
    Args:
        ...
        model_dir: Where weights are cached. ``None`` resolves at load time,
            ...
            Nexus is frozen, and capturing it at import time would bake in a
        ...
        language: Forced language code, or ``None`` to auto-detect. Forcing it
            skips a detection pass and avoids Nexus occasionally deciding that a
    ...
    def _load_profile(self, profile: ModelProfile) -> WhisperModel:
        ...
        Prefers a directory Nexus downloaded itself, which is what a packaged
