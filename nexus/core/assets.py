"""Fetching the large files Nexus needs but does not ship.

The speech model and the voice are 200 MB between them. Bundling them into the
installer would work, but it makes every download of Nexus 200 MB heavier and
...
* **Treat a stall as a failure.** A connection that stops delivering bytes but
  ...
  twice on the machine Nexus was built on.
...
# CTranslate2 model repositories, keyed by the profile names in nexus.stt.whisper.
...
def whisper_assets(model: str) -> list[Asset]:
    ...
    Returns:
        The four files, or an empty list for a model Nexus has no repository for.
...
def voice_assets(voice: str) -> list[Asset]:
    ...
    Raises:
        AssetError: If the voice name is not one Nexus knows how to fetch.
    """
...
def required(*, model: str, voice: str) -> list[Asset]:
    """Everything Nexus needs before it can start, minus what is already present.

    Returns:
        The missing assets, in download order. Empty means Nexus is ready to run
        offline.
    """
...
def _fetch_once(
    ...
    request = urllib.request.Request(asset.url, headers={"User-Agent": "Nexus/1.0"})
    if offset:
