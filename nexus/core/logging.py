...
Nexus's INFO messages on screen. Someone who installed Nexus wants a clean startup
...
LOG_FILE_NAME: Final = "nexus.log"
MAX_LOG_BYTES: Final = 1_000_000
LOG_BACKUPS: Final = 2


def log_path() -> Path:
    """Where Nexus writes its log."""
...
def configure(
    ...
    *,
    ...
    Args:
        level: Level for Nexus's own loggers on the console.
...
def _add_file_handler(root: logging.Logger) -> None:
    ...
    An unwritable disk should not stop Nexus from running; it only means a
