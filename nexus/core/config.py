...
class Settings:
    """Everything Nexus's behaviour can be tuned by.

    Attributes:
        llm_provider: Which service Nexus prefers. Others with a stored key are
        ...
        silence_ms: Pause that ends a turn in hands-free mode. Below roughly
            600 ms Nexus interrupts people who stop to think; much above 1000 ms
        ...
        allow_screen: Whether Nexus may look at the screen when a question needs
        ...
        allow_browser: Whether Nexus may open pages and scroll the browser the
            user already has open. It never clicks anything, and it acts only
            on a window that is already in front.
        show_orb: Whether to float the animated orb on screen while Nexus
        ...
        reasoning: How hard the model should think before answering, for models
            ...
            Thinking is latency the user hears as silence, so Nexus turns it off
...
def load_settings(env_file: Path | None = None) -> Settings:
    ...
    return Settings(
        llm_provider=_get_str("NEXUS_LLM_PROVIDER", "groq").lower(),
        llm_model=_get_str("NEXUS_LLM_MODEL"),
        api_keys=_read_api_keys(),
        input_device=_get_optional_str("NEXUS_INPUT_DEVICE"),
        output_device=_get_optional_str("NEXUS_OUTPUT_DEVICE"),
        always_on_mic=_get_bool("NEXUS_ALWAYS_ON_MIC", default=True),
        device=_get_str("NEXUS_DEVICE", "auto").lower(),
        voice=_get_str("NEXUS_VOICE", "en_US-norman-medium"),
        speech_speed=_get_float("NEXUS_SPEECH_SPEED", 1.0),
        silence_ms=int(_get_float("NEXUS_SILENCE_MS", 800)),
        vad_threshold=_get_float("NEXUS_VAD_THRESHOLD", 0.5),
        allow_screen=_get_bool("NEXUS_ALLOW_SCREEN", default=True),
        allow_browser=_get_bool("NEXUS_ALLOW_BROWSER", default=True),
        show_orb=_get_bool("NEXUS_SHOW_ORB", default=True),
