"""Where Nexus's files live, in a source checkout and in a packaged build.
...
===============  ==========================  =============================
                 Running from source         Running as Nexus.exe
===============  ==========================  =============================
Code and         the repository              a temporary folder PyInstaller
bundled assets                               unpacks and later deletes
Downloaded       ``models/``, ``voices/``    ``%LOCALAPPDATA%\\Nexus``
Settings, key,   ``%LOCALAPPDATA%\\Nexus``      ``%LOCALAPPDATA%\\Nexus``
name, logs
===============  ==========================  =============================
...
# parents[2] is the repository root: nexus/core/paths.py -> nexus/core -> ev -> root.
# Only meaningful when running from source; see bundle_dir.
_SOURCE_ROOT: Final = Path(__file__).resolve().parents[2]


def is_frozen() -> bool:
    """Whether Nexus is running from a packaged executable."""
    return bool(getattr(sys, "frozen", False))


def bundle_dir() -> Path:
    """Directory holding read-only files shipped with Nexus.
