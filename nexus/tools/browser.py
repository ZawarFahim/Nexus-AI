...
So Nexus types. Opening a page hands a URL to the default browser exactly as
clicking a link does; navigation synthesises the keystrokes a person would
press. Two consequences worth stating plainly:

**Nexus acts on the focused window and never steals focus.** Windows restricts
...
or, worse, their terminal. If the front window is not a browser, Nexus says so
...
# Time for a newly launched browser to come up before Nexus claims success.
...
def open_tool() -> Tool:
    ...
    return Tool(
        ...
        # a single string -- rejected by its own validator, after which Nexus said
